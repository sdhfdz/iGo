...
public class RegisterNextActivity extends BaseActivity {
            ...
            if (result == SMSSDK.RESULT_COMPLETE) {
                ...
                try {
                    ...
                    if (status > 0 && !TextUtils.isEmpty(des)) {
                        if (status == 468) {
                            Toast.makeText(RegisterNextActivity.this, "您输入的验证码不正确", Toast.LENGTH_SHORT).show();

                        } else {
                            Toast.makeText(RegisterNextActivity.this, des, Toast.LENGTH_SHORT).show();
    ...
    class MyResponseInterface extends AsyncHttpResponseHandler{
        ...
        public void onFailure(int i, Header[] headers, byte[] bytes, Throwable throwable) {
                Toast.makeText(RegisterNextActivity.this,"网络错误",Toast.LENGTH_SHORT);
        }
    }
    ...
    public void register_button(View v) {
        ...
        if(TextUtils.isEmpty(passwd_user)||TextUtils.isEmpty(passwd_confirm)){
            Toast.makeText(RegisterNextActivity.this,"密码不能为空",Toast.LENGTH_SHORT).show();
        }else if(!passwd_confirm.equals(passwd_user)){
            Toast.makeText(RegisterNextActivity.this,"两次输入的密码不一致",Toast.LENGTH_SHORT).show();
        }else {
    ...
    public void post_data(){
        ...
        ahc.post(GlobalCheckPost.SERVER_URL+"/mobile_profile/register", params, new MyResponseInterface());
